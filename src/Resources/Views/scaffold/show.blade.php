...
@section('title', trans('crud::panel.show').' '.trans("$crud->name::panel.$crud->name") )

@section('page-heading'){{trans('crud::panel.show').' '.trans("$crud->name::panel.$crud->name")}} @endsection
            ...
            @foreach($crud->fields as $field)
                @if(strpos($field->visible, 's') !== false)
                    ...
                    @if(view()->exists("$crud->name::admin.inputTypes.$field->type"))
                        ...
                        @else
                            @include("crud::admin.scaffold.inputTypes.$field->type",['field'=>$field,'crud'=>$crud, 'item'=>$item])
                    @endif
                @endif
            @endforeach
